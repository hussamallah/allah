...
import { getAllSpotlightResults } from '@/lib/spotlightStageTestUtils';

export default function SpotlightAllDiagnosesPage() {
  const router = useRouter();
  const allResults = getAllSpotlightResults();
  ...
  const handleViewDiagnosis = (dominantStage: string, maskStage: string) => {
    router.push(`/chamber/spotlight/quiz?dominant=${dominantStage}&mask=${maskStage}`);
  };
  return (
    <div className="min-h-screen bg-black text-white p-6">
      <div className="max-w-6xl mx-auto">
        <h1 className="text-4xl font-bold text-center mb-8 text-pink-400">
          All Spotlight Diagnoses ({allDiagnoses.length})
        </h1>
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          {allDiagnoses.map(({ dominantStage, maskStage, result, key }) => (
            <div 
              key={key}
              className="bg-gray-900 border border-gray-700 p-4 rounded-lg hover:border-pink-500 cursor-pointer transition-colors"
        ...
        <div className="mt-8 text-center">
          <button
            onClick={() => router.push('/chamber/spotlight/quiz')}
