...
import { getAllWandererResults } from '@/lib/wandererStageTestUtils';

export default function WandererAllDiagnosesPage() {
  const router = useRouter();
  const allResults = getAllWandererResults();
  ...
  const handleViewDiagnosis = (dominantStage: string, maskStage: string) => {
    router.push(`/chamber/wanderer/quiz?dominant=${dominantStage}&mask=${maskStage}`);
  };
  return (
    <div className="min-h-screen bg-black text-white p-6">
      <div className="max-w-6xl mx-auto">
        <h1 className="text-4xl font-bold text-center mb-8 text-yellow-400">
          All Wanderer Diagnoses ({allDiagnoses.length})
        </h1>
        ...
        <div className="mt-8 text-center">
          <button
            onClick={() => router.push('/chamber/wanderer/quiz')}
