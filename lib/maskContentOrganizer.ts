...
export interface MaskContentBlock {
  stage: string;
  mask: string;
  content: string;
}

export interface OrganizedMaskContent {
  [stage: string]: {
    [mask: string]: string;
  };
}
...
/**
 * Organizes raw mask content into the proper structure
 * @param rawContent - The 25,000 words of content to organize
 * @returns Organized content ready to paste into the JSON file
 */
export function organizeMaskContent(rawContent: string): OrganizedMaskContent {
  const stages = ['shapeshifter', 'chameleon', 'mask', 'veil', 'mirror'];
  const masks = ['shapeshifter', 'chameleon', 'mask', 'veil', 'mirror'];
  
  const organized: OrganizedMaskContent = {};
 ...
 * @param organizedContent - The organized content from organizeMaskContent
 * @returns Complete JSON structure ready to replace the current file
 */
export function generateMaskResultsJSON(organizedContent: OrganizedMaskContent): string {
  const stages = ['shapeshifter', 'chameleon', 'mask', 'veil', 'mirror'];
  const masks = ['shapeshifter', 'chameleon', 'mask', 'veil', 'mirror'];
  ...
  for (let stageIndex = 0; stageIndex < stages.length; stageIndex++) {
    ...
    for (let maskIndex = 0; maskIndex < masks.length; maskIndex++) {
      ...
      const existingResult = maskResults.maskResults[stage]?.[mask];
...
/**
 * Processes raw mask content and returns the complete JSON
 * @param rawContent - The raw content to process
 * @returns Complete JSON string ready to write to file
 */
export function processMaskContent(rawContent: string): string {
  const organized = organizeMaskContent(rawContent);
  return generateMaskResultsJSON(organized);
}

/**
 * Validates mask content
 * @param content - The content to validate
 * @returns Validation result
 */
export function validateMaskContent(content: string): {
  isValid: boolean;
  wordCount: number;
...
/**
 * Creates a template for mask content
 * @returns Template string
 */
export function createMaskContentTemplate(): string {
  return `MASK NODE CONTENT TEMPLATE

This should contain 25,000 words of content organized into 25 different stage/mask combinations for the Mask archetype.

The stages are: shapeshifter, chameleon, mask, veil, mirror
The masks are: shapeshifter, chameleon, mask, veil, mirror
...
Format: MASK NODE: [STAGE] | [MASK] (1000 words) followed by the content.`;
} 