...
export interface OrganizedProviderContent {
  [stage: string]: {
    [mask: string]: string;
  };
}
...
/**
 * Organizes raw provider content into the proper structure
 * @param rawContent - The 25,000 words of content to organize
 * @returns Organized content ready to paste into the JSON file
 */
export function organizeProviderContent(rawContent: string): OrganizedProviderContent {
  const stages = ['giver', 'sustainer', 'provider', 'source', 'fountain'];
  const masks = ['giver', 'sustainer', 'provider', 'source', 'fountain'];
  
  const organized: OrganizedProviderContent = {};
 ...
 * @param organizedContent - The organized content from organizeProviderContent
 * @returns Complete JSON structure ready to replace the current file
 */
export function generateProviderResultsJSON(organizedContent: OrganizedProviderContent): string {
  const stages = ['giver', 'sustainer', 'provider', 'source', 'fountain'];
  const masks = ['giver', 'sustainer', 'provider', 'source', 'fountain'];
  ...
  for (let stageIndex = 0; stageIndex < stages.length; stageIndex++) {
    ...
    for (let maskIndex = 0; maskIndex < masks.length; maskIndex++) {
      ...
      const existingResult = providerResults.providerResults[stage]?.[mask];
...
/**
 * Processes raw provider content and returns the complete JSON
 * @param rawContent - The raw content to process
 * @returns Complete JSON string ready to write to file
 */
export function processProviderContent(rawContent: string): string {
  const organized = organizeProviderContent(rawContent);
  return generateProviderResultsJSON(organized);
}

/**
 * Validates provider content
 * @param content - The content to validate
 * @returns Validation result
 */
export function validateProviderContent(content: string): {
  isValid: boolean;
  wordCount: number;
...
/**
 * Creates a template for provider content
 * @returns Template string
 */
export function createProviderContentTemplate(): string {
  return `PROVIDER NODE CONTENT TEMPLATE

This should contain 25,000 words of content organized into 25 different stage/mask combinations for the Provider archetype.

The stages are: giver, sustainer, provider, source, fountain
The masks are: giver, sustainer, provider, source, fountain
...
Format: PROVIDER NODE: [STAGE] | [MASK] (1000 words) followed by the content.`;
} 