...
export interface SovereignContentBlock {
  stage: string;
  mask: string;
  content: string;
}

export interface OrganizedSovereignContent {
  [stage: string]: {
    [mask: string]: string;
  };
}
 ...
 * @param organizedContent - The organized content from organizeSovereignContent
 * @returns Complete JSON structure ready to replace the current file
 */
export function generateSovereignResultsJSON(organizedContent: OrganizedSovereignContent): string {
  const stages = ['watcher', 'commander', 'sovereign', 'crown', 'throne'];
  const masks = ['watcher', 'commander', 'sovereign', 'crown', 'throne'];
  ...
  for (let stageIndex = 0; stageIndex < stages.length; stageIndex++) {
    ...
    for (let maskIndex = 0; maskIndex < masks.length; maskIndex++) {
      ...
      const existingResult = sovereignResults.sovereignResults[stage]?.[mask];
...
/**
 * Processes raw sovereign content and returns the complete JSON
 * @param rawContent - The raw content to process
 * @returns Complete JSON string ready to write to file
 */
export function processSovereignContent(rawContent: string): string {
  const organized = organizeSovereignContent(rawContent);
  return generateSovereignResultsJSON(organized);
}

/**
 * Validates sovereign content
 * @param content - The content to validate
 * @returns Validation result
 */
export function validateSovereignContent(content: string): {
  isValid: boolean;
  wordCount: number;
...
/**
 * Creates a template for sovereign content
 * @returns Template string
 */
export function createSovereignContentTemplate(): string {
  return `SOVEREIGN NODE CONTENT TEMPLATE
...
The stages are: watcher, commander, sovereign, crown, throne
The masks are: watcher, commander, sovereign, crown, throne
