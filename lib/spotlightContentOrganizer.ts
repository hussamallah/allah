...
export interface SpotlightContentBlock {
  stage: string;
  mask: string;
  content: string;
}

export interface OrganizedSpotlightContent {
  [stage: string]: {
    [mask: string]: string;
  };
}
...
/**
 * Organizes raw spotlight content into the proper structure
 * @param rawContent - The 25,000 words of content to organize
 * @returns Organized content ready to paste into the JSON file
 */
export function organizeSpotlightContent(rawContent: string): OrganizedSpotlightContent {
  const stages = ['hider', 'mimic', 'performer', 'beacon', 'projector'];
  const masks = ['hider', 'mimic', 'performer', 'beacon', 'projector'];
  
  const organized: OrganizedSpotlightContent = {};
 ...
 * @param organizedContent - The organized content from organizeSpotlightContent
 * @returns Complete JSON structure ready to replace the current file
 */
export function generateSpotlightResultsJSON(organizedContent: OrganizedSpotlightContent): string {
  const stages = ['hider', 'mimic', 'performer', 'beacon', 'projector'];
  const masks = ['hider', 'mimic', 'performer', 'beacon', 'projector'];
  ...
  for (let stageIndex = 0; stageIndex < stages.length; stageIndex++) {
    ...
    for (let maskIndex = 0; maskIndex < masks.length; maskIndex++) {
      ...
      const existingResult = spotlightResults.spotlightResults[stage]?.[mask];
...
/**
 * Processes raw spotlight content and returns the complete JSON
 * @param rawContent - The raw content to process
 * @returns Complete JSON string ready to write to file
 */
export function processSpotlightContent(rawContent: string): string {
  const organized = organizeSpotlightContent(rawContent);
  return generateSpotlightResultsJSON(organized);
}

/**
 * Validates spotlight content
 * @param content - The content to validate
 * @returns Validation result
 */
export function validateSpotlightContent(content: string): {
  isValid: boolean;
  wordCount: number;
...
/**
 * Creates a template for spotlight content
 * @returns Template string
 */
export function createSpotlightContentTemplate(): string {
  return `SPOTLIGHT NODE CONTENT TEMPLATE

This should contain 25,000 words of content organized into 25 different stage/mask combinations for the Spotlight archetype.

The stages are: hider, mimic, performer, beacon, projector
The masks are: hider, mimic, performer, beacon, projector
...
Format: SPOTLIGHT NODE: [STAGE] | [MASK] (1000 words) followed by the content.`;
} 