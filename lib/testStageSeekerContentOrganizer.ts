...
export interface TestStageSeekerContentBlock {
  stage: string;
  mask: string;
  content: string;
}

export interface OrganizedTestStageSeekerContent {
  [stage: string]: {
    [mask: string]: string;
  };
}
...
/**
 * Organizes raw testStageSeeker content into the proper structure
 * @param rawContent - The 25,000 words of content to organize
 * @returns Organized content ready to paste into the JSON file
 */
export function organizeTestStageSeekerContent(rawContent: string): OrganizedTestStageSeekerContent {
  ...
  const organized: OrganizedTestStageSeekerContent = {};
 ...
 * @param organizedContent - The organized content from organizeTestStageSeekerContent
 * @returns Complete JSON structure ready to replace the current file
 */
export function generateTestStageSeekerResultsJSON(organizedContent: OrganizedTestStageSeekerContent): string {
  const stages = ['questioner', 'edgeFlincher', 'testStageSeeker', 'seeker', 'finder'];
  const masks = ['questioner', 'edgeFlincher', 'testStageSeeker', 'seeker', 'finder'];
  
  let json = '{\n  "testStageSeekerResults": {\n';
  ...
  for (let stageIndex = 0; stageIndex < stages.length; stageIndex++) {
    ...
    for (let maskIndex = 0; maskIndex < masks.length; maskIndex++) {
      ...
      const existingResult = testStageSeekerResults.testStageSeekerResults[stage]?.[mask];
...
/**
 * Processes raw testStageSeeker content and returns the complete JSON
 * @param rawContent - The raw content to process
 * @returns Complete JSON string ready to write to file
 */
export function processTestStageSeekerContent(rawContent: string): string {
  const organized = organizeTestStageSeekerContent(rawContent);
  return generateTestStageSeekerResultsJSON(organized);
}

/**
 * Validates testStageSeeker content
 * @param content - The content to validate
 * @returns Validation result
 */
export function validateTestStageSeekerContent(content: string): {
  isValid: boolean;
  wordCount: number;
...
/**
 * Creates a template for testStageSeeker content
 * @returns Template string
 */
export function createTestStageSeekerContentTemplate(): string {
  return `TEST STAGE SEEKER NODE CONTENT TEMPLATE
...
The stages are: questioner, edgeFlincher, testStageSeeker, seeker, finder
The masks are: questioner, edgeFlincher, testStageSeeker, seeker, finder
