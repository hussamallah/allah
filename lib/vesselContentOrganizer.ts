...
export interface VesselContentBlock {
  stage: string;
  mask: string;
  content: string;
}

export interface OrganizedVesselContent {
  [stage: string]: {
    [mask: string]: string;
  };
}
...
/**
 * Organizes raw vessel content into the proper structure
 * @param rawContent - The 25,000 words of content to organize
 * @returns Organized content ready to paste into the JSON file
 */
export function organizeVesselContent(rawContent: string): OrganizedVesselContent {
  const stages = ['helper', 'carrier', 'vessel', 'bridge', 'channel'];
  const masks = ['helper', 'carrier', 'vessel', 'bridge', 'channel'];
  
  const organized: OrganizedVesselContent = {};
 ...
 * @param organizedContent - The organized content from organizeVesselContent
 * @returns Complete JSON structure ready to replace the current file
 */
export function generateVesselResultsJSON(organizedContent: OrganizedVesselContent): string {
  const stages = ['helper', 'carrier', 'vessel', 'bridge', 'channel'];
  const masks = ['helper', 'carrier', 'vessel', 'bridge', 'channel'];
  ...
  for (let stageIndex = 0; stageIndex < stages.length; stageIndex++) {
    ...
    for (let maskIndex = 0; maskIndex < masks.length; maskIndex++) {
      ...
      const existingResult = vesselResults.vesselResults[stage]?.[mask];
...
/**
 * Processes raw vessel content and returns the complete JSON
 * @param rawContent - The raw content to process
 * @returns Complete JSON string ready to write to file
 */
export function processVesselContent(rawContent: string): string {
  const organized = organizeVesselContent(rawContent);
  return generateVesselResultsJSON(organized);
}

/**
 * Validates vessel content
 * @param content - The content to validate
 * @returns Validation result
 */
export function validateVesselContent(content: string): {
  isValid: boolean;
  wordCount: number;
...
/**
 * Creates a template for vessel content
 * @returns Template string
 */
export function createVesselContentTemplate(): string {
  return `VESSEL NODE CONTENT TEMPLATE

This should contain 25,000 words of content organized into 25 different stage/mask combinations for the Vessel archetype.

The stages are: helper, carrier, vessel, bridge, channel
The masks are: helper, carrier, vessel, bridge, channel
...
Format: VESSEL NODE: [STAGE] | [MASK] (1000 words) followed by the content.`;
} 