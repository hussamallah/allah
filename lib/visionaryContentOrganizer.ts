...
export interface VisionaryContentBlock {
  stage: string;
  mask: string;
  content: string;
}

export interface OrganizedVisionaryContent {
  [stage: string]: {
    [mask: string]: string;
  };
}
...
/**
 * Organizes raw visionary content into the proper structure
 * @param rawContent - The 25,000 words of content to organize
 * @returns Organized content ready to paste into the JSON file
 */
export function organizeVisionaryContent(rawContent: string): OrganizedVisionaryContent {
  const stages = ['dreamer', 'escapist', 'visionary', 'creator', 'manifestor'];
  const masks = ['dreamer', 'escapist', 'visionary', 'creator', 'manifestor'];
  
  const organized: OrganizedVisionaryContent = {};
 ...
 * @param organizedContent - The organized content from organizeVisionaryContent
 * @returns Complete JSON structure ready to replace the current file
 */
export function generateVisionaryResultsJSON(organizedContent: OrganizedVisionaryContent): string {
  const stages = ['dreamer', 'escapist', 'visionary', 'creator', 'manifestor'];
  const masks = ['dreamer', 'escapist', 'visionary', 'creator', 'manifestor'];
  ...
  for (let stageIndex = 0; stageIndex < stages.length; stageIndex++) {
    ...
    for (let maskIndex = 0; maskIndex < masks.length; maskIndex++) {
      ...
      const existingResult = visionaryResults.visionaryResults[stage]?.[mask];
...
/**
 * Processes raw visionary content and returns the complete JSON
 * @param rawContent - The raw content to process
 * @returns Complete JSON string ready to write to file
 */
export function processVisionaryContent(rawContent: string): string {
  const organized = organizeVisionaryContent(rawContent);
  return generateVisionaryResultsJSON(organized);
}

/**
 * Validates visionary content
 * @param content - The content to validate
 * @returns Validation result
 */
export function validateVisionaryContent(content: string): {
  isValid: boolean;
  wordCount: number;
...
/**
 * Creates a template for visionary content
 * @returns Template string
 */
export function createVisionaryContentTemplate(): string {
  return `VISIONARY NODE CONTENT TEMPLATE

This should contain 25,000 words of content organized into 25 different stage/mask combinations for the Visionary archetype.

The stages are: dreamer, escapist, visionary, creator, manifestor
The masks are: dreamer, escapist, visionary, creator, manifestor
...
Format: VISIONARY NODE: [STAGE] | [MASK] (1000 words) followed by the content.`;
} 