...
export interface WandererContentBlock {
  stage: string;
  mask: string;
  content: string;
}

export interface OrganizedWandererContent {
  [stage: string]: {
    [mask: string]: string;
  };
}
...
/**
 * Organizes raw wanderer content into the proper structure
 * @param rawContent - The 25,000 words of content to organize
 * @returns Organized content ready to paste into the JSON file
 */
export function organizeWandererContent(rawContent: string): OrganizedWandererContent {
  const stages = ['nomad', 'drifter', 'explorer', 'adventurer', 'settler'];
  const masks = ['nomad', 'drifter', 'explorer', 'adventurer', 'settler'];
  
  const organized: OrganizedWandererContent = {};
 ...
 * @param organizedContent - The organized content from organizeWandererContent
 * @returns Complete JSON structure ready to replace the current file
 */
export function generateWandererResultsJSON(organizedContent: OrganizedWandererContent): string {
  const stages = ['nomad', 'drifter', 'explorer', 'adventurer', 'settler'];
  const masks = ['nomad', 'drifter', 'explorer', 'adventurer', 'settler'];
  ...
  for (let stageIndex = 0; stageIndex < stages.length; stageIndex++) {
    ...
    for (let maskIndex = 0; maskIndex < masks.length; maskIndex++) {
      ...
      const existingResult = wandererResults.wandererResults[stage]?.[mask];
...
/**
 * Processes raw wanderer content and returns the complete JSON
 * @param rawContent - The raw content to process
 * @returns Complete JSON string ready to write to file
 */
export function processWandererContent(rawContent: string): string {
  const organized = organizeWandererContent(rawContent);
  return generateWandererResultsJSON(organized);
}

/**
 * Validates wanderer content
 * @param content - The content to validate
 * @returns Validation result
 */
export function validateWandererContent(content: string): {
  isValid: boolean;
  wordCount: number;
...
/**
 * Creates a template for wanderer content
 * @returns Template string
 */
export function createWandererContentTemplate(): string {
  return `WANDERER NODE CONTENT TEMPLATE

This should contain 25,000 words of content organized into 25 different stage/mask combinations for the Wanderer archetype.

The stages are: nomad, drifter, explorer, adventurer, settler
The masks are: nomad, drifter, explorer, adventurer, settler
...
Format: WANDERER NODE: [STAGE] | [MASK] (1000 words) followed by the content.`;
} 